...
- Inside the `/beers` route, call the `getBeers()` method (the **PunkAPI** provides this method, and you can find more about it [here][def]). **Calling the `.getBeers()` method returns a promise that should be resolved with an array of 25 beers**.
...
[def]: https://www.npmjs.com/package/punkapi-javascript-wrapper#getbeersoptions
